...
export function NewEventForm() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const successKey = published ? 'event-created-published' : 'event-created-draft'
      router.push(`/admin/events?success=${successKey}`)
      router.refresh()
    } catch (caughtError: unknown) {
      setError(caughtError instanceof Error ? caughtError.message : 'Unable to create event.')
      setSaving(false)
    }
  }
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        />
        <p className="mt-1 text-xs text-gray-500">If no image is uploaded, a default event image will be used.</p>
      </div>
      ...
      <div className="flex items-center gap-3">
        <Button type="submit" size="sm" loading={saving}>Save Event</Button>
