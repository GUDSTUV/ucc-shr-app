  ...
  return (
    <button
      ...
      className={
        variant === 'unstyled'
          ? className ?? ''
          : `inline-flex items-center justify-center gap-2 rounded-md font-medium font-sans transition-all active:scale-[0.97] disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer ${variants[variant]} ${sizes[size]} ${fullWidth ? 'w-full' : ''} ${className ?? ''}`.trim()
      }
      {...rest}
    >
